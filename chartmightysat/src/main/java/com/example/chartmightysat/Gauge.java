...
public abstract class Gauge extends View {
    ...
    /** the max range in MightySatmeter, {@code default = 100} */
    private float maxSpeed = 100f;
    /** the min range in MightySatmeter, {@code default = 0} */
    private float minSpeed = 0f;
    /**
     * the last speed which you set by {@link #speedTo(float)}
     * or {@link #speedTo(float, long)} or {@link #speedPercentTo(int)},
     * or if you stop MightySatmeter By {@link #stop()} method.
     */
    ...
    private boolean MightySatmeterTextRightToLeft = false;
    ...
    private void initAttributeSet(Context context, AttributeSet attrs) {
       ...
       // speedTextPaint.setColor(a.getColor(R.styleable.Gauge_sv_speedTextColor, speedTextPaint.getColor()));
        setSpeedTextColor(a.getColor(R.styleable.Gauge_sv_speedTextColor, speedTextPaint.getColor()));
        ...
        MightySatmeterTextRightToLeft = a.getBoolean(R.styleable.Gauge_sv_textRightToLeft, MightySatmeterTextRightToLeft);
    ...
    private void checkMightySatmeterPercent() {
     ...
     * this method must call in subMightySatmeter's {@code onDraw} method.
...
//                , r.top - speedUnitTextBitmap.getHeight()+200 , speedUnitTextBitmapPaint);
        canvas.drawBitmap(speedUnitTextBitmap, r.left - speedUnitTextBitmap.getWidth() *.5f + r.width() *.5f
                , r.top - speedUnitTextBitmap.getHeight() *.65f , speedUnitTextBitmapPaint);
    }

    /**
     * clear {@link #speedUnitTextBitmap} and draw speed and unit Text
     * taking into consideration {@link #MightySatmeterTextRightToLeft} and {@link #unitUnderSpeedText}.
     */
    private void updateSpeedUnitTextBitmap(String speedText) {
        ...
        if (unitUnderSpeedText) {
            ...
            float speedX;
            float unitX;
            if (isMightySatmeterTextRightToLeft()) {
    ...
    /**
     * stop MightySatmeter and run tremble if {@link #withTremble} is true.
     ...
     * the MightySatmeter to {@link #maxSpeed}.
     *
     * @see #realSpeedTo(float)
     * @see #slowDown()
     */
     ...
     * the MightySatmeter to {@link #minSpeed}.
     *
     * @see #realSpeedTo(float)
     * @see #speedUp()
     */
    ...
    /**
     * to make MightySatmeter some real.
     * <br>
...
//                : (speed + mad < minSpeed) ? minSpeed - speed : mad;
        trembleAnimator = ValueAnimator.ofFloat(currentSpeed, speed );
     ...
     * used to add some reality to MightySatmeter.<br>
    ...
    /**
     * get max speed in MightySatmeter, default max speed is 100.
     * @return max speed.
     *
     * @see #getMinSpeed()
    ...
    /**
     * get min speed in MightySatmeter, default min speed is 0.
     * @return min speed.
     *
     * @see #getMaxSpeed()
     ...
     * <a href="https://github.com/anastr/SpeedView/wiki/Usage#control-division-of-the-MightySatmeter">section</a> changed.
     ...
     */
    public boolean isMightySatmeterTextRightToLeft() {
        return MightySatmeterTextRightToLeft;
    }

    /**
     * to support Right To Left Text.
     * @param MightySatmeterTextRightToLeft true to flip text right to left.
     */
    public void setMightySatmeterTextRightToLeft(boolean MightySatmeterTextRightToLeft) {
        this.MightySatmeterTextRightToLeft = MightySatmeterTextRightToLeft;
        if (!attachedToWindow)
            return;
        updateBackgroundBitmap();
        invalidate();
     ...
     * for speed Text and MightySatmeter Text.
     ...
     * used in condition : {@code if (MightySatmeter.getSection() == MightySatmeter.LOW_SECTION)}.
     */
